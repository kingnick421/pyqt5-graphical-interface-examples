...
class DrawingLabel(QLabel):
    ...
    def drawHangmanBody(self, painter):
        ...
        if "right_arm" in self.part_list:
            right_arm = QLine(int((self.width / 2)) + 60, 85, int((self.width / 2) + 50), int((self.height / 2) + 30))
            pen = QPen(Qt.black, 3, Qt.SolidLine)
            painter.setPen(pen)
            painter.drawLine(right_arm)
        if "left_arm" in self.part_list:
            left_arm = QLine(int((self.width / 2) + 62), 85, int((self.width / 2) + 72), int((self.height / 2) + 30))
            painter.drawLine(left_arm)
        if "right_leg" in self.part_list:
            right_leg = QLine(int((self.width / 2) + 60), 135, int((self.width / 2) + 50), int((self.height / 2) + 75))
            painter.drawLine(right_leg)
        if "left_leg" in self.part_list:
            left_leg = QLine(int((self.width / 2) + 62), 135, int((self.width / 2) + 72), int((self.height / 2) + 75))
...
class Hangman(QMainWindow):
    ...
    def displayDialogs(self, text):
        ...
        """
        if text == "win":
            message = QMessageBox().question(self, "Win!", "You Win!\nNEW GAME?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        elif text == "game_over":
            message = QMessageBox().question(self, "Game Over", "Game Over\nNEW GAME?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
